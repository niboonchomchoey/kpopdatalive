...
const StatisticsCount = () => {
    ...
    useEffect(() => {
        const fetchStatistics = async () => {
            const response = await fetch(`https://www.googleapis.com/youtube/v3/videos?part=statistics&id=${process.env.VIDEO_ID_2}&key=${process.env.API_KEY}`)
...
export default function MainCard() {
    ...
    useEffect(() => {
        ...
        const fetchThumbnail = async () => {
            const response = await fetch(`https://www.googleapis.com/youtube/v3/videos?part=snippet&id=${process.env.VIDEO_ID_2}&key=${process.env.API_KEY}`)
            const data = await response.json()
